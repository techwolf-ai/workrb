...
from workbench.config import BenchmarkConfig
from workbench.logging import setup_logger
from workbench.metrics.reporting import format_results
from workbench.models.base import ModelInterface
from workbench.registry import TaskRegistry
from workbench.results import (
    BenchmarkMetadata,
    BenchmarkResults,
    MetricsResult,
    TaskResultMetadata,
...
from workbench.tasks.abstract.base import Language, Task
...
class WorkBench:
    ...
    def __init__(self, tasks: Sequence[Task]):
        ...
        """
        self.tasks: Sequence[Task] = tasks
    ...
    def _init_checkpointing(
        ...
        if not force_restart and config and config.has_checkpoint():
            ...
            config.validate_results_contained_in_tasks(results=existing_results, tasks=self.tasks)
    ...
    def _run_pending_work(
        ...
        for work_info in work_by_task.values():
            ...
            logger.info(f"Completed {run_idx} / {self._get_total_evaluations()} evaluations. ")
    ...
    def run(
        ...
        results, pending_work = self._init_checkpointing(
            config=config,
            model=model,
        ...
        logger.info(f"{'=' * 60}")
        logger.info(
            f"Pending work: {len(pending_work)} / {self._get_total_evaluations()} evaluations"
        )
        ...
        results = self._run_pending_work(
            config=config,
        ...
        results.metadata.resumed_from_checkpoint = len(pending_work) < sum(
            len(task.languages) for task in self.tasks
        )
    ...
    def _get_total_evaluations(self) -> int:
        ...
        return sum(len(task.languages) for task in self.tasks)
    ...
    def get_tasks_overview(self) -> str:
        """Get information about configured tasks as a formatted string summary."""
        # Calculate summary statistics
        num_tasks = len(self.tasks)
        task_groups = {task.task_group for task in self.tasks if task.task_group}
